...
window.addEventListener("load", function() {
	...
	function populateArticle(jsonResult) {
		...
		if (jsonResult.hasOwnProperty("data")) {
			...
			myCount.textContent = "Watson sees (from trained)...";
			article.appendChild(myCount);
			detectedObjectsTable(classified, article);
			if (jsonResult.hasOwnProperty("dataDefault")) {
				let classified = jsonResult.dataDefault.classes;
				const myCount = document.createElement("h3");
				myCount.textContent = "Watson sees (from default)...";
				article.appendChild(myCount);
				detectedObjectsTable(classified, article);
			}
		} else {
