...
const credentials = vcapServices.getCredentials({
...
const visual_recognition = new VisualRecognitionV3({
	version: "2018-03-19",
	iam_apikey: "akMfgtJbMwcMorXPK0wxROIfjTo-f_hifXZDjlM0ercW"
});
		...
		if (err) {
			...
			visual_recognition.classify(params, function(err, res) {
				if (err) {
					...
					const labelsvr = JSON.parse(JSON.stringify(res)).images[0]
						.classifiers[0];
					visual_recognition.classify(
						...
						function(err, res) {
							if (err) {
								...
								const labelDefault = JSON.parse(JSON.stringify(res)).images[0]
									.classifiers[0];
								result.send({ data: labelsvr, dataDefault: labelDefault });
							}
						}
					);
				}
